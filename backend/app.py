...
def analyse():
    # Создаем уникальный UUID сессии, чтобы не перезатереть чужие файлы (если одновременно несколько человек использует сервис)
    sessionUuid = str(uuid.uuid4())
    
    # Получение параметров из запроса
    files = request.files
    user_images_folder = os.path.join(os.getcwd(), 'images', sessionUuid)
    
    if not os.path.exists(user_images_folder):
        os.makedirs(user_images_folder)
    
    for file in files.values():
        file.save(os.path.join(user_images_folder, file.filename))
    ...
    results_df['Category_Description'] = results_df['Category'].apply(lambda x: category_descriptions.get(str(x), "Неизвестная категория"))
    ...
    results_df['Sugar_Absence_Compliance'] = results_df.apply(check_sugar_absence, axis=1)
    ...
    # для поиска возраста
    def find_age_recommendation(text, age_df):
        for _, row in age_df.iterrows():
            keyword = row['Надпись']
            age_recommendation = row['Возраст']
            if re.search(r'\b' + re.escape(keyword) + r'\b', text, re.IGNORECASE):
                return age_recommendation
        return "Не обнаружено"

    age_recommendation = find_age_recommendation(all_text_rus, age_df)

    if 'Age_Recommendation' not in results_df.columns:
        results_df['Age_Recommendation'] = age_recommendation
    else:
        results_df.at[0, 'Age_Recommendation'] = age_recommendation
    ...
    def calculate_score(row):
        ...
        one_point_deductions = [
            ...
            'Sugar_Absence_Compliance', 'Percentage_Presence', 'Protein_Compliance', 
            'Fat_Compliance'
        ]
    ...
    results_df['Final_Score'] = results_df.iloc[:1].apply(calculate_score, axis=1)
    # print(results_df)
    # сохранямсмя
    ...
    response = jsonify({"result": json_data})
